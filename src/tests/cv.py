...
for c in cnts:
    ...
    cv2.fillPoly(filtered_particles, pts =[convex_hull], color=(255,255,255))
cv2.namedWindow('aplicação convex hull', cv2.WINDOW_NORMAL)
cv2.imshow('aplicação convex hull', filtered_particles)

circles = cv2.HoughCircles(filtered_particles, cv2.HOUGH_GRADIENT, 1, 150, param1 = 255,
...
if circles is not None:
    ...
    for (a, b, r) in circles[0, :]:
        ...
        cv2.circle(original_image, center_coordinates, r, (0, 255, 0), 2)

        # Draw a small circle (of radius 1) to show the center.
        cv2.circle(original_image, (a, b), 1, (0, 0, 255), 3)
