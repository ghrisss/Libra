...
class VisionJob():
    ...
    def rivet_mark(self, rivet_image):
        ...
        filtered_rivet_1 = VisionController.areaParticleFilter(floodfill_rivet, maximum_particle=55)
        ...
        if FRAME.get('SHOW'):
            cv2.imshow('análise de marcação padrão', filtered_rivet_3)
        
        _, hierarchy_list = cv2.findContours(filtered_rivet_3, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def rivet_presence(self, rivet_image):
        ...
        if crop_filtered_roi is not None:
            ...
            if rivet_circles is not None:
                ...
                for i, (a, b, r) in enumerate(rivet_circles[0, :]):
                    ...
                    self.rivet_mark(rivet_image)
                    self.rivet_presence(rivet_image)
